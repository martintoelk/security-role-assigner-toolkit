...
export interface RoleChangeOperation { action: ChangeAction; target: Target; role: SecurityRole; }
...
export function createRoleChangeOperations(request: PreviewRequest): RoleChangeOperation[] {
    ...
    for (const target of request.targets) for (const role of selectedRoles) {
        ...
        if (request.action === "add") {
            if (!assigned.some((assignedRole) => assignedRole.id === role.id)) operations.push({ action: "add", target, role });
            continue;
        }
